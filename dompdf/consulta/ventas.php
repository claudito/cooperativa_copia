...
$requisc  =  new Venta_cab();
$requisd  =  new Venta_det();
$id  =  $_GET['id'];
...
<h1 class="center">REGISTRO DE VENTAS</h1>
...
<td><?php echo $requisc->consulta($_GET['id'],'nombres'); ?></td>
</tr>
<tr>
...
foreach ($requisd->lista($_GET['id']) as $key => $value): ?>
<?php 

                $sub=round($value['precio_unitario']-$value['precio_unitario']/1.18,2);
                $igv=round($value['precio_unitario']-$sub,2); 
...
<td class="detalle-td center"><?php echo round($value['precio_unitario'],2); ?></td>
<td class="detalle-td center"><?php $costo=($value['cantidad']*$value['precio_unitario']);  echo $costo; ?></td>
...
<?php 
  ...
  $suma_uni =  $suma_uni+$value['precio_unitario'];
