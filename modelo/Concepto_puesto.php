...
$query      =  "SELECT cp.id,c.descripcion concepto,puesto.codigo puesto,p.nombres,p.apellidos,p.tipo 
tipo_comerciante,c.tipo tipo_concepto FROM concepto_puesto as cp 
...
$conexion   =  new Conexion();
$db         =  $conexion->get_conexion();
$query      =  "SELECT cp.id,cp.id_concepto,cp.codigo_puesto,c.nombres,c.apellidos,puesto.codigo numero_puesto,
UPPER(puesto.estado)estado,UPPER(puesto.tipo)tipo FROM concepto_puesto as cp
...
INNER JOIN puesto ON cp.codigo_puesto=puesto.id
